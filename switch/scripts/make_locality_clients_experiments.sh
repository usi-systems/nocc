...
for locality in 0.5 0.625 0.75 0.875 1.0
do
    for TD in 50
    do
        for N in 16
            ...
            do
                ...
                do
                    ...
                    python gen_experiment.py --stdout-log \
                        -c  "../pygotthard/load_generator.py -i 1000 -n $lcl_cnt -t %think% -d %runtime% -T A(x,x)W(x,RND)|R(x) -p %write_p% -l %l %h %p" \
                          "../pygotthard/load_generator.py -i 1100 -n $rem_cnt -t %think% -d %runtime% -T A(y,y)W(y,RND)|R(y) -p %write_p% -l %l %h %p" \
                          "../pygotthard/load_generator.py -i 2000 -n $lcl_cnt -t %think% -d %runtime% -T A(y,y)W(y,RND)|R(y) -p %write_p% -l %l %h %p" \
                          "../pygotthard/load_generator.py -i 2200 -n $rem_cnt -t %think% -d %runtime% -T A(x,x)W(x,RND)|R(x) -p %write_p% -l %l %h %p" \
