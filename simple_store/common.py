...
class ReqMsg:
    ...
    def __init__(self, binstr=None, cl_id=0, req_id=0, r_key=0, r_version=0, w_key=0, w_value=''):
        if binstr is not None:
            self.unpack(binstr)
        else:
            self.cl_id, self.req_id, self.r_key, self.r_version, self.w_key, self.w_value = cl_id, req_id, r_key, r_version, w_key, w_value

    def unpack(self, binstr):
        self.cl_id, self.req_id, self.r_key, self.r_version, self.w_key, self.w_value = struct.unpack(reqmsg_fmt, binstr)

    def pack(self):
        return struct.pack(reqmsg_fmt, self.cl_id, self.req_id, self.r_key, self.r_version, self.w_key, self.w_value)

    def __str__(self):
        return "ReqMsg(cl_id=%d, req_id=%d, r_key=%d, r_version=%d, w_key=%d, w_value='%s')" % (self.cl_id, self.req_id, self.r_key, self.r_version, self.w_key, self.w_value.rstrip('\0'))
