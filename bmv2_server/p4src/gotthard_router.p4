...
#include "loop_tables_generated.p4"
...
action rewrite_addr(in bit<48> src_mac_addr, in bit<32> dst_addr, in bit<48> dst_mac_addr, in bit<9> out_port) {
    ipv4.srcAddr = ipv4.dstAddr;
    ipv4.dstAddr = dst_addr;
    ethernet.srcAddr = src_mac_addr;
    ethernet.dstAddr = dst_mac_addr;
